 ...
 * @param {string} action shell command
 */
const cloneMultiple = async (paths, branch) => {
...
const bowerGit = async options => {
    ...
    if (list.length) {
        const spinner = ora().start('Cloning multiple');
        const multi = await cloneMultiple(list, branch);
        spinner.succeed(`Bower components ${chalk.bold.green(multi.join(', '))} has been replaced by its git repository`);
    } else {
        if (!folderPath) {
